...
import { NEW_PRODUCT_UPDATE_RESET } from "../constants/newProductConstants";

const NewProductEditScreen = ({ match, history }) => {
  ...
  const newProductUpdate = useSelector((state) => state.newProductUpdate);
  const {
  ...
  useEffect(() => {
    if (successUpdate) {
      dispatch({ type: NEW_PRODUCT_UPDATE_RESET });
  ...
  return (
    <>
      <Link to="/newproduct" className="btn btn-light my-3">
        Go Back
      </Link>
      <FormContainer>
        <h1>Edit Product</h1>
...
export default NewProductEditScreen;
